...
from PyQt6.QtWidgets import (
    ...
    QInputDialog, QToolBar, QSizePolicy, QStyle 
)
...
ABOUT_INFO = {
    ...
    "version": "0.2a", 
    "authors": "Куренков С.С., Кирасиров А.И.",
    "copyright": "© 2025 egg's Team. Все права защищены.",
    "description": (
        "KAXa - это комплексное приложение, разработанное в рамках курсового проекта. "
        "Оно объединяет в себе функционал для ведения заметок и управления задачами (To-Do лист)."
    )
}
    ...
    def data(self, index, role=Qt.ItemDataRole.DisplayRole):
        ...
        if role == Qt.ItemDataRole.FontRole:
            ...
            if file_info.isFile() and file_info.suffix().lower() == 'md':
                return (f"{file_info.baseName()}") # Если что вернуть "— "
...
class CombinedApp(QMainWindow):
    def __init__(self):
        ...
        # Создание Toolbar для кнопки "О программе"
        self.main_toolbar = QToolBar("Main Toolbar")
        self.main_toolbar.setMovable(False)
        self.main_toolbar.setIconSize(QSize(16,16)) # Для компактности иконки
        self.addToolBar(Qt.ToolBarArea.TopToolBarArea, self.main_toolbar)

        # Spacer для выравнивания кнопки "О программе" по правому краю
        spacer = QWidget()
        ...
        self.about_action = QAction(about_icon, "", self) # Пустой текст для отображения только иконки
        ...
        self.update_window_title() # Инициализируем обновление title

    def show_about_dialog(self):
        info = ABOUT_INFO # Используем глобальный словарь ABOUT_INFO
        QMessageBox.about(self,
                          f"О программе {info['program_name']}",
...
if __name__ == '__main__':
    ...
    QTabWidget::pane { border: 1px solid #3A3A3A; /* border-top: none; */ } /* Убрал верхнюю границу панели, если тулбар сверху */
    QTabBar::tab {
    ...
    QTabBar::tab:selected {
        ...
        color: #E0E0E0;
        /* border-color: #3A3A3A; */ /* Убрано, чтобы лучше сливалось с QTabWidget::pane */
    }
    QTabBar::tab:hover {
    ...
    QToolBar {
        background-color: #202020; /* Match window background */
        border-bottom: 1px solid #3A3A3A; /* Separator from tabs */
        padding: 2px; /* Minimal padding */
        spacing: 3px; /* Spacing between items */
    }
    QToolBar QToolButton { /* Actions in a toolbar are QToolButtons */
        background-color: transparent; /* Blend with toolbar */
        color: #E0E0E0; /* Icon color if it's font-based, or for text */
        border: none; 
        padding: 4px;
    ...
    QToolBar QToolButton:hover {
        background-color: #383838; /* Subtle hover */
    }
    QToolBar QToolButton:pressed {
        background-color: #4A90D9; /* Accent color on press */
        color: #FFFFFF;
    }
    """
